...
export const SEED_SERIES: SeedSeries[] = [
  {
    ...
    cards: [
      ...
      { slug: "m7-double-pont-nouvelle-generation", name: "M7 — Le Double Pont Nouvelle Génération", description: "La toute dernière génération de voitures à deux étages, écrans d'information et climatisation modernisée, progressivement déployée sur le réseau.", rarity: "COMMON", combatStatsEnabled: true, combatStats: { power: 40, reliability: 75, charm: 55 } },
      { slug: "classe-55-manoeuvrier-diesel-des-triages", name: "Classe 55 — Le Manœuvrier Diesel des Triages", description: "Elle ne quitte presque jamais le triage, mais sans elle, aucune rame ne serait jamais complète au départ.", rarity: "COMMON", combatStatsEnabled: true, combatStats: { power: 35, reliability: 80, charm: 40 } },
      { slug: "hle19-traxx-polyvalente-du-fret", name: "HLE 19 — La Traxx Polyvalente", description: "Cousine de la HLE 18, elle passe sans broncher du train de voyageurs au long convoi de marchandises.", rarity: "UNCOMMON", combatStatsEnabled: true, combatStats: { power: 65, reliability: 70, charm: 50 } },
    ],
  },
  {
    slug: "gares-et-lieux",
    ...
    cards: [
      ...
      { slug: "ottignies-noeud-du-brabant-wallon", name: "Ottignies — Le Nœud du Brabant Wallon", description: "Plus de 21 000 voyageurs y montent chaque jour de semaine, ce qui en fait la gare la plus fréquentée du Brabant wallon.", rarity: "UNCOMMON" },
      { slug: "louvain-carrefour-etudiant", name: "Louvain — Le Carrefour Étudiant", description: "Plus de 31 000 voyageurs embarqués par jour de semaine, portés par l'une des plus grandes universités du pays juste à côté des quais.", rarity: "UNCOMMON" },
      { slug: "gand-saint-pierre-geante-discrete", name: "Gand-Saint-Pierre — La Géante Discrète", description: "Avec près de 49 000 voyageurs montés par jour de semaine, elle devance Anvers-Central sans jamais s'en vanter.", rarity: "RARE" },
      { slug: "bruxelles-midi-porte-internationale", name: "Bruxelles-Midi — La Porte Internationale", description: "Plus de 58 000 voyageurs embarqués chaque jour de semaine, et le point de départ vers Paris, Londres ou Amsterdam en Eurostar et Thalys.", rarity: "EPIC" },
      { slug: "bruxelles-nord-poumon-du-reseau", name: "Bruxelles-Nord — Le Poumon du Réseau", description: "La gare la plus fréquentée du pays : environ 59 000 voyageurs y montent chaque jour de semaine, autant à elle seule que les trois cents plus petites gares belges réunies.", rarity: "LEGENDARY" },
    ],
  },
  {
    ...
    cards: [
      ...
      { slug: "ait-increvable-de-service", name: "AIT — L'Increvable de Service", description: "Astreinte, nuit, jour férié : AIT intervient sur le matériel en toutes circonstances, boîte à outils toujours prête dans le coffre.", rarity: "COMMON", combatStatsEnabled: true, combatStats: { power: 55, reliability: 70, charm: 40 } },
      { slug: "le-repartiteur-materiel", name: "Le Répartiteur Matériel", description: "Chaque matin, il décide quelle rame partira où : un puzzle de garages, de kilométrages et d'entretiens qu'il résout avant le lever du jour.", rarity: "UNCOMMON", combatStatsEnabled: true, combatStats: { power: 50, reliability: 85, charm: 45 } },
      { slug: "dpd-pompier-du-reseau", name: "DPD — Le Pompier du Réseau", description: "On ne le voit jamais sur les quais, seulement sur les avis d'incident : DPD débarque dès qu'une panne menace de bloquer une ligne entière.", rarity: "UNCOMMON", combatStatsEnabled: true, combatStats: { power: 75, reliability: 65, charm: 50 } },
      { slug: "tli-oeil-discret-de-la-ligne", name: "TLI — L'Œil Discret de la Ligne", description: "Il connaît chaque aiguillage et chaque passage à niveau de son secteur, et repère un rail fatigué avant qu'il ne le devienne vraiment.", rarity: "UNCOMMON", combatStatsEnabled: true, combatStats: { power: 45, reliability: 80, charm: 45 } },
      { slug: "rts-chef-dorchestre-des-sillons", name: "RTS — Le Chef d'Orchestre des Sillons", description: "Dans la tour de contrôle, tout le monde l'appelle simplement RTS. Il suit chaque train à la seconde près et réorganise des dizaines de sillons dès qu'un imprévu s'en mêle.", rarity: "RARE", combatStatsEnabled: true, combatStats: { power: 70, reliability: 90, charm: 55 } },
      { slug: "paco-maitre-des-compositions", name: "PACO — Le Maître des Compositions", description: "Avant chaque départ, PACO vérifie que la rame a la bonne longueur, les bonnes voitures, dans le bon ordre. Une erreur, et c'est tout un quai qui déborde.", rarity: "RARE", combatStatsEnabled: true, combatStats: { power: 60, reliability: 88, charm: 50 } },
      { slug: "le-permanencier-veilleur-de-nuit-du-reseau", name: "Le Permanencier — Le Veilleur de Nuit du Réseau", description: "Quand tout le monde dort, il reste en ligne : le permanencier centralise les incidents de la nuit et décide, seul, qui réveiller.", rarity: "EPIC", combatStatsEnabled: true, combatStats: { power: 65, reliability: 95, charm: 60 } },
      { slug: "rcca-enqueteur-de-lombre", name: "RCCA — L'Enquêteur de l'Ombre", description: "Après chaque incident sérieux, RCCA épluche les rapports, remonte la chaîne des causes, et rédige des conclusions que personne n'aime lire mais que tout le monde applique.", rarity: "LEGENDARY", combatStatsEnabled: true, combatStats: { power: 80, reliability: 92, charm: 65 } },
    ],
  },
  {
    slug: "vie-de-quai",
